...
def test_examples_from_task(cat_age: int, dog_age: int, expected: list[int]) -> None:
    assert get_human_age(cat_age, dog_age) == expected


def test_cat_age_grows_every_4_years_after_24() -> None:
    assert get_human_age(28, 0)[0] == get_human_age(24, 0)[0] + 1
    assert get_human_age(32, 0)[0] == get_human_age(28, 0)[0] + 1


def test_dog_age_grows_every_5_years_after_24() -> None:
...
def test_invalid_types_raise_error() -> None:
    with pytest.raises(TypeError):
        get_human_age("10", 5)  # type: ignore[arg-type]
