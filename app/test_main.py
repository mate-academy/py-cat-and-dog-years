...
def test_first_threshold_exactly():
    """Тест з віком рівно 15 років"""
    assert get_human_age(15, 15) == [1, 1]


def test_second_threshold_range():
...
def test_second_threshold_exactly():
    """Тест з віком рівно 24 роки (15 + 9)"""
    assert get_human_age(24, 24) == [2, 2]


def test_third_threshold_range():
    """Тест з віком після 24 років"""
    assert get_human_age(25, 25) == [2, 2]
    assert get_human_age(27, 27) == [2, 2]


def test_cat_dog_difference():
    """Тест де коти і собаки мають різний вік у людських роках"""
    # Кіт: 28 років = 1 (за 15) + 1 (за 9) + 1 (за 4) = 3 роки
    # Собака: 28 років = 1 (за 15) + 1 (за 9) + 0 (4 роки недостатньо для наступного порогу 5) = 2 роки
    assert get_human_age(28, 28) == [3, 2]


def test_cat_multiple_extra_years():
    """Тест з котами, що мають багато додаткових років"""
    # Кіт: 32 роки = 1 (за 15) + 1 (за 9) + 2 (за 8, кожні 4 роки) = 4 роки
    # Собака: 32 роки = 1 (за 15) + 1 (за 9) + 1 (за 5, залишилося 3 роки) = 3 роки
    assert get_human_age(32, 32) == [4, 3]


def test_dog_multiple_extra_years():
    """Тест з собаками, що мають багато додаткових років"""
    # Кіт: 36 років = 1 (за 15) + 1 (за 9) + 3 (за 12, кожні 4 роки) = 5 років
    # Собака: 36 років = 1 (за 15) + 1 (за 9) + 2 (за 10, кожні 5 років) = 4 роки
