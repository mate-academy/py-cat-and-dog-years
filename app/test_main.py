...
@pytest.mark.parametrize(
    ...
    ids=[
        ...
        "get third human year for cat upper value",
        "test multiply extra human years"
    ]
)
def test_can_sum(cat_age: int, dog_age: int, result: list[int]) -> None:
    assert (get_human_age(cat_age, dog_age) == result),\
        (f"Result of func for cat_age={cat_age} and "
         f"dog_age={dog_age} should be equal to {result}")
